...
std::shared_ptr<Shape> Space::findPtr(cpShape* shape) const {
	...
	auto it = find_if(shapes.begin(), shapes.end(),
	                  [&shape](const std::shared_ptr<Shape>& s) { return *s == shape; });
...
void Space::segmentQuery(Vect a, Vect b, Layers layers, Group group, SegmentQueryFunc func) const {
	SegmentQueryData data = { this, func };
	cpSpaceSegmentQuery(space, a, b, static_cast<cpLayers>(layers), static_cast<cpGroup>(group),
	                    segmentQueryFunc, &data);
}

std::shared_ptr<Shape> Space::segmentQueryFirst(Vect a, Vect b, Layers layers, Group group,
                                                SegmentQueryInfo* const info) const {
	cpSegmentQueryInfo i;
	auto rtn = cpSpaceSegmentQueryFirst(space, a, b, static_cast<cpLayers>(layers),
	                                    static_cast<cpGroup>(group), &i);
	if (info) {
		info->t = i.t;
		info->n = i.n;
	}
...
std::shared_ptr<Shape> Space::pointQueryFirst(Vect p, Layers layers, Group group) const {
	return findPtr(cpSpacePointQueryFirst(space, p, static_cast<cpLayers>(layers),
...
void Space::addCollisionHandler(CollisionType a, CollisionType b,
	...
	cpSpaceAddCollisionHandler(space, a, b, begin == nullptr ? nullptr : helperBegin,
	                           preSolve == nullptr ? nullptr : helperPreSolve,
	                           postSolve == nullptr ? nullptr : helperPostSolve,
	                           separate == nullptr ? nullptr : helperSeparate, data);
}
