...
public:
	...
	/// Returns the squared length. Faster than Vect::length() when you only need to compare
	/// lengths.
	...
	/// Returns a normalized copy or the zero vector if the vector was already the zero vector.
	...
	/// Uses complex multiplication to return a copy rotated by v. Scaling will occur if the vector
	/// isn't a unit vector.
	...
	/// 2D vector cross product analog. The cross product of 2D vectors results in a 3D vector with
	/// only a z component. This function returns the value along the z-axis.
	...
	/// Returns the squared distance between v1 and v2. Faster than Vect::dist when you only need to
	/// compare distances.
...
bool operator!=(const Vect& lhs, const Vect& rhs);
Vect operator*(const Vect& lhs, Float v);
Vect operator/(const Vect& lhs, Float v);
Vect operator*(Float v, const Vect& rhs);
Vect operator/(Float v, const Vect& rhs);
